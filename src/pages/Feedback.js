...
/* ------------------------------------------------------------------ */
/* Ferrofluid background (inlined — no separate file/import needed)    */
/* ------------------------------------------------------------------ */
...
const prepColors = (input) => {
  const base = (input && input.length ? input : ["#7c3aed", "#3b82f6", "#14b8a6"]).slice(0, MAX_COLORS);
...
function Ferrofluid({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, { vertex: ferroVertex, fragment: ferroFragment, uniforms });
