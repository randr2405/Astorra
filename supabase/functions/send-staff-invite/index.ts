...
serve(async (req) => {
  ...
  try {
    ...
    // Server-to-server calls (e.g. payfast-notify, after a staff seat
    // payment confirms) authenticate with the Supabase service role key
    // directly, rather than a Firebase-issued user token — there's no
    // signed-in browser session in a payment webhook. Only our own
    // backend ever has this secret, so an exact match is sufficient to
    // trust the caller and skip the Firebase/business lookup below.
